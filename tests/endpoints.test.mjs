...
test("generated endpoints satisfy counts, manifest integrity, and latest endpoint shape", async () => {
  const endpoints = await validateOutputDirectory(docsDirectory);
  ...
  for (const entry of manifest.endpoints) {
    assert.ok(endpoints[entry.path], `manifest entry ${entry.path} should exist on disk`);
  }
});

test("all public files under docs/api end in .json and homepage uses relative asset paths", async () => {
  const topLevelEntries = await readdir(apiDirectory, { withFileTypes: true });

  for (const entry of topLevelEntries) {
    if (entry.isFile()) {
      assert.ok(entry.name.endsWith(".json"));
    }
  }
